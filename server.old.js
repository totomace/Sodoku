...
// --- Middlewares ---
app.use(express.json({ limit: '10kb' })); // Giới hạn request size
...
function writeDB(data) {
    try {
        // Ghi bất đồng bộ (async) để không block server
...
// Throttle broadcast để tránh spam
let broadcastTimeout = null;
function broadcastUserList() {
    if (broadcastTimeout) return; // Đang chờ broadcast
    
    broadcastTimeout = setTimeout(() => {
        const userListArray = Object.values(connectedUsers);
        io.emit('updateUserList', userListArray);
        broadcastTimeout = null;
    }, 100); // Chỉ broadcast tối đa 10 lần/giây
}
...
app.post('/api/save-game', (req, res) => { 
    ...
    if (!username || !mode || score === undefined) {
        return res.status(400).json({ success: false, message: 'Thiếu thông tin game' }); 
    }
    const db = readDB(); 
    const newGame = { 
        username: username, 
    ...
    // Giới hạn lịch sử: chỉ giữ 100 game gần nhất mỗi user
    const MAX_HISTORY = 100;
    db.gameHistory = db.gameHistory.slice(-MAX_HISTORY * 2); // x2 vì có nhiều user
...
setInterval(() => {
    const now = Date.now();
    const GAME_TIMEOUT = 15 * 60 * 1000; // 15 phút không hoạt động
    
    for (const roomName in activeGames) {
        const game = activeGames[roomName];
        if (now - game.startTime > GAME_TIMEOUT) {
            console.log(`Dọn game cũ: ${roomName}`);
            delete activeGames[roomName];
        }
    }
}, 5 * 60 * 1000);

// Tối ưu timer - chỉ chạy khi có game
setInterval(() => {
    // Bỏ qua nếu không có game nào đang chơi
    const gameKeys = Object.keys(activeGames);
    if (gameKeys.length === 0) return;
    const now = Date.now();
    // Dùng for...of thay vì for...in (nhanh hơn)
    for (const roomName of gameKeys) {
        const game = activeGames[roomName];
        if (!game || !game.currentTurn) continue; // Skip game không hợp lệ
